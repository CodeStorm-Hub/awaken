...
> [!IMPORTANT]
> **Dependencies**: Implementing this will require adding several heavy dependencies: `flutter_map` (for map rendering), `flutter_background_geolocation` (for rock-solid background GPS), `turf` (for local spatial math), and native activity recognition plugins (for anti-cheat).
...
Raw GPS data is noisy and easily manipulated. The core gameplay loop will rely on these mathematical and algorithmic layers, powered by specific Flutter packages:

### Background GPS Tracking
*   **`flutter_background_geolocation`**: We will use this premium package (by Transistor Software) as the gold standard for maintaining rock-solid GPS tracking even when the user locks their phone or switches apps mid-run.

### Local Geometry Prep (Client-Side Math)
*   **`turf` Package (Dart)**: Before sending any coordinates to the backend, we will use the Dart port of Turf.js to handle local spatial math efficiently. 
*   **Validating the "Bounded Loop"**: We will use `turf` to calculate the Haversine distance between $P_{start}$ and $P_{end}$. A run is considered a loop if $Distance \le 20\text{m}$.
*   **GPS Drift and Path-Snapping**: We will apply a real-time Kalman filter to smooth the incoming noisy GPS coordinates, and run the **Ramer-Douglas-Peucker (RDP)** line simplification algorithm (via `turf` or custom Isolate) post-run to clean up the polygon.
...
**Crucial for Map Games:** We will NOT try to calculate multi-user polygon slicing on the phone. The client will only send raw path coordinates to the backend. All heavy spatial relationships will be handled directly in PostgreSQL using the PostGIS extension.
...
### Automated Tests
*   **Unit Tests**: Dart tests to verify the `turf` Haversine distance logic, the RDP simplification array output, and the speed-capping constraints.
