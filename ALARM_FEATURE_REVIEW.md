...
### Separately reported bug: tapping "Complete Squats" on the notification didn't open the camera screen

Device test: the alarm fired and showed correctly in the notification shade (`Wake Up Tax Due! / Complete 10 squats to dismiss your alarm.`), but tapping the **Complete Squats** action button did nothing — `ActiveAlarmScreen` never opened.

Two things going on here, only one of which is a bug:

1. **Not a bug:** the notification appeared in the shade instead of auto-launching full-screen. Android only force-launches a full-screen-intent notification as a full-screen Activity when the device is **locked or the screen is off**; if the screen is already on/unlocked (as in the reported screenshot — quick-settings tiles pulled down, meaning the device was actively in use), the OS deliberately downgrades it to a normal heads-up/shade notification instead of interrupting the user. This is documented Android behavior, not something to "fix."
2. **Real bug, fixed:** `AlarmNotificationService._onForegroundTap` (`lib/core/services/alarm_notification_service.dart`) read `navigatorKey.currentContext` and **silently did nothing** if it was `null`. On a cold start (app fully closed, which matches the report), `flutter_local_notifications` can deliver the action tap via this exact callback — fired during plugin initialization, *before* `runApp()`'s widget tree exists — rather than via `getNotificationAppLaunchDetails()` (the path `main()`'s `getInitialRoute()` already handles correctly). When that race hit, the tap event was simply dropped: no navigation, no error, nothing — exactly the reported symptom.

**Fix:** `_onForegroundTap` now falls back to stashing the target route (the same mechanism the background-isolate handler already used) instead of silently dropping the tap when there's no navigator context yet. `AwakenApp.initState()` (`lib/app.dart`) now also consumes any stashed pending route on its very first post-frame callback, not only on an `AppLifecycleState.resumed` transition — closing the gap where a route stashed mid-`main()` might otherwise wait for a resume event that may never fire right after a genuine cold start.
