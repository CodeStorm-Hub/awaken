...
void main() {
  ...
  setUpAll(() {
    ...
    // flutter_local_notifications v18+ requires tests to manually register a
    // platform implementation (real app runs get this for free from the
    // generated plugin registrant) — without it, FlutterLocalNotifications-
    // Platform.instance is an uninitialized `late` field.
  ...
  GeoPointEntity movePointBySpeed(
    GeoPointEntity from,
    double speedKmh,
    Duration duration,
  ) {
  ...
  group('Tier 1: Feature Coverage (F1-F7)', () {
    ...
    test(
      ...
      () {
        fakeAsync((async) {
          ...
          final baseTime = DateTime.now();
          mockGeolocator.feedPosition(
            createPos(lat: 40.7128, lng: -74.0060, time: baseTime),
          );
          async.flushMicrotasks();
          ...
          async.flushMicrotasks();
          expect(
            container.read(activeRunProvider).distanceMeters,
            greaterThan(50.0),
          );
        });
      },
    ...
    test('6. F2: Slow run (e.g. 8 km/h) is not flagged as over-speed.', () {
      ...
      for (var i = 1; i <= 6; i++) {
        points.add(
          movePointBySpeed(points.last, 8.0, const Duration(seconds: 1)),
        );
      }
    ...
    test(
      ...
      () {
        ...
        for (var i = 1; i <= 6; i++) {
          points.add(
            movePointBySpeed(points.last, 30.0, const Duration(seconds: 1)),
          );
        }
    ...
    test(
      ...
      () {
        fakeAsync((async) {
          ...
          expect(
            container.read(activeRunProvider).result?.outcome,
    ...
    test(
      ...
      () {
        ...
        // 1 spike
        points.add(
          movePointBySpeed(points.last, 40.0, const Duration(seconds: 1)),
        );
        // 5 normal pings
        for (var i = 0; i < 5; i++) {
          points.add(
    ...
    test(
      '15. F3: Densified (not RDP-simplified) vertices are used for database capture.',
      () {
        fakeAsync((async) {
          final container = ProviderContainer(
            overrides: [
          ...
          final userTerritory = fakeTerritoryRepository.territories.firstWhere(
            (t) => t.userId == 'user-1',
          );
    ...
    test(
      '16. F4: Closed loop (start/end distance <= 50m) with min duration (2m) and min distance (200m) and area (>50m²) is valid.',
      () {
        final baseTime = DateTime.now();
    ...
    test(
      '17. F4: Loop not closed (start/end distance > 50m) is classified as normal workout (not claimed).',
      () {
        final baseTime = DateTime.now();
    ...
    test(
      '17b. F4: Loop closed mid-run with overrun past start is claimed on Stop.',
      () {
        fakeAsync((async) {
          final container = ProviderContainer(
            overrides: [
          ...
          expect(
            fakeTerritoryRepository.territories.any(
              (t) => t.userId == 'user-1',
            ),
            isTrue,
          );
    ...
    test(
      '18. F4: Closed loop but duration below minimum is classified as `invalidatedTooShort`.',
      () {
        final baseTime = DateTime.now();
        final loop = createRectangleLoop(
          startLat: 40.7128,
    ...
    test(
      ...
      () async {
        // Seed rival
        final rivalLoop = createRectangleLoop(
          startLat: 40.7135,
          startLng: -74.0055,
          widthMeters: 100,
          heightMeters: 100,
        ...
        fakeTerritoryRepository.territories.add(
          TerritoryEntity(
            id: 'rival-t',
            userId: 'rival-1',
        ...
        // Overlapping claim
        final userLoop = createRectangleLoop(
          startLat: 40.7130,
          startLng: -74.0050,
          widthMeters: 100,
          heightMeters: 100,
        ...
        final rival = fakeTerritoryRepository.territories.firstWhere(
          (t) => t.userId == 'rival-1',
        );
    ...
    test(
      ...
      () async {
        ...
        final rivals = fakeTerritoryRepository.territories.where(
          (t) => t.userId == 'rival-1',
        );
    ...
    test('26. F6: Territory last defended within 7 days does not decay.', () {
      fakeAsync((async) {
        ...
        expect(
          fakeTerritoryRepository.territories.first.areaSqMeters,
          equals(2500.0),
        );
      });
    });
    ...
    test(
      ...
      () {
        fakeAsync((async) {
          ...
          fakeTerritoryRepository.simulateDecay(
    ...
    test(
      ...
      () async {
        ...
        await expectLater(
          TerritoryDecayNotificationService.notifyIfDecaying(territoryCount: 1),
          completes,
        );
      },
    );
    ...
    test(
      ...
      () async {
        ...
        expect(
          fakeTerritoryRepository.territories.first.lastDefendedAt.isAfter(
            originalTime,
          ),
          isTrue,
        );
    ...
    test(
      ...
      () async {
        ...
        final leaderboard = await fakeTerritoryRepository
    ...
    test(
      ...
      () async {
        ...
        final nearby = await fakeTerritoryRepository.getNearbyLeaderboard(
          viewer,
        );
    ...
    test(
      ...
      () async {
        ...
        final subscription = fakeTerritoryRepository.watchTerritories().listen(
          updates.add,
        );
  ...
  group('Tier 2: Boundary & Corner Cases', () {
    ...
    test(
      ...
      () {
        fakeAsync((async) {
          ...
          mockGeolocator.feedPosition(
            createPos(lat: 40.7128, lng: -74.0060, time: DateTime.now()),
          );
          async.flushMicrotasks();
          ...
          expect(
            container.read(activeRunProvider).result?.points.length,
            equals(1),
          );
          expect(
            container.read(activeRunProvider).result?.outcome,
    ...
    test(
      ...
      () {
        fakeAsync((async) {
          ...
          mockGeolocator.feedPosition(
            createPos(lat: 40.7128, lng: -74.0060, time: now),
          );
          async.flushMicrotasks();
          mockGeolocator.feedPosition(
            createPos(
      ...
      () {
        fakeAsync((async) {
          ...
          expect(
            ...
            equals(RunSessionStatus.tracking),
          );
        });
      },
    );
    ...
    test(
      ...
      () {
        ...
        for (var i = 0; i < 6; i++) {
          ...
          points.add(
            movePointBySpeed(points.last, 24.99, const Duration(seconds: 1)),
          );
        }
        expect(RunValidationService.isSustainedOverSpeed(points), isFalse);
      },
    ...
    test(
      ...
      () {
        ...
        for (var i = 0; i < 6; i++) {
          points.add(
            movePointBySpeed(points.last, 25.01, const Duration(seconds: 1)),
          );
        }
        expect(RunValidationService.isSustainedOverSpeed(points), isTrue);
      },
    ...
    test(
      ...
      () {
        ...
        points.add(
          movePointBySpeed(points.last, 10.0, const Duration(seconds: 1)),
        );
        // Intervals 2-6: fast
        for (var i = 0; i < 4; i++) {
          points.add(
    ...
    test(
      ...
      () {
        ...
        for (var i = 0; i < 5; i++) {
          points.add(
            movePointBySpeed(points.last, 500.0, const Duration(seconds: 1)),
          );
        }
        expect(RunValidationService.isSustainedOverSpeed(points), isTrue);
      },
    ...
    test(
      ...
      () {
        ...
        final ptMid1 = GeoPointEntity(
          latitude: 40.7128 + 0.0000268,
          longitude: -74.0070,
          timestamp: baseTime.add(const Duration(seconds: 30)),
        );
        // Collinear mid-point but offset by 3.01m in latitude
        final ptMid2 = GeoPointEntity(
          latitude: 40.7128 + 0.0000271,
          longitude: -74.0070,
          timestamp: baseTime.add(const Duration(seconds: 30)),
        );

        final simplified1 = RdpSimplifier.simplify([
          ptStart,
          ptMid1,
          ptEnd,
        ], 3.0);
        final simplified2 = RdpSimplifier.simplify([
          ptStart,
          ptMid2,
          ptEnd,
        ], 3.0);
    ...
    test('54. F4: Run duration one second below the minimum is invalid.', () {
      ...
      final loop = createRectangleLoop(
        ...
        heightMeters: 60,
        startTime: baseTime,
        interval: const Duration(seconds: 29),
      );
      final outcome = RunValidationService.classify(
        points: loop,
        distanceMeters: 240,
    ...
    test(
      ...
      () async {
        ...
        expect(
          fakeTerritoryRepository.territories.where(
            (t) => t.userId == 'rival-1',
          ),
          isEmpty,
        );
    ...
    test(
      ...
      () async {
        ...
        final user = fakeTerritoryRepository.territories.firstWhere(
          (t) => t.userId == 'user-1',
        );
    ...
    test(
      ...
      () async {
        ...
        expect(
          fakeTerritoryRepository.territories.any((t) => t.userId == 'rival-1'),
          isTrue,
        );
      },
    );
      ...
      () {
        fakeAsync((async) {
          ...
          final t1 = TerritoryEntity(
            ...
            polygons: [loop1],
            areaSqMeters: 2500.0,
            lastDefendedAt: DateTime.now().subtract(
              const Duration(days: 4, hours: 23, minutes: 45),
            ),
            isOwnedByCurrentUser: true,
          );
    ...
    test(
      ...
      () async {
        ...
        final defensePt = GeoPointEntity(
    ...
    test(
      ...
      () async {
        ...
        final leaderboard = await fakeTerritoryRepository.getNearbyLeaderboard(
          viewer,
        );
        expect(leaderboard, isEmpty);
      },
  ...
  group('Tier 3: Cross-Feature Combinations', () {
    ...
    test(
      ...
      () {
        fakeAsync((async) {
          ...
          final initialLoop = createRectangleLoop(
            startLat: 40.7128,
            startLng: -74.0060,
            widthMeters: 60,
            heightMeters: 60,
          ...
          mockGeolocator.feedPosition(
            createPos(
              lat: loop[0].latitude,
              lng: loop[0].longitude,
              time: loop[0].timestamp,
              accuracy: 0.1,
              speed: 8.0,
            ),
          );
          ...
          expect(state.result?.outcome, equals(RunOutcome.territoryClaimed));
          expect(
            fakeTerritoryRepository.territories
                .firstWhere((t) => t.userId == 'user-1')
                .polygons
                .length,
            equals(1),
          );
        ...
        fakeAsync((async) {
          ...
          final container = ProviderContainer(
            overrides: [
              ...
              viewerLocationProvider.overrideWith(
                FakeViewerLocationNotifier.new,
              ),
              leaderboardModeProvider.overrideWith(
                FakeLeaderboardModeNotifier.new,
              ),
            ],
          );
          addTearDown(container.dispose);
    ...
    test(
      ...
      () {
        fakeAsync((async) {
          final loopUser = createRectangleLoop(
            startLat: 40.7128,
            startLng: -74.0060,
            widthMeters: 60,
            heightMeters: 60,
          ...
          ); // Large age offset to cause shrink

          // Reset rival's area and defender time so rival doesn't decay
          final rivalIdx = fakeTerritoryRepository.territories.indexWhere(
            (t) => t.userId == 'rival-1',
          );
          ...
          expect(
            userTerritory.lastDefendedAt.isAfter(
        ...
        fakeAsync((async) {
          ...
          fakeTerritoryRepository.territories.add(
            TerritoryEntity(
              ...
              polygons: [loopUser],
              areaSqMeters: 3600.0,
              lastDefendedAt: DateTime.now().subtract(const Duration(days: 6)),
              isOwnedByCurrentUser: true,
            ),
          );
          ...
          final baseTime = DateTime.now();
          final path = createRectangleLoop(
            startLat: 40.7130,
            startLng: -74.0060,
            widthMeters: 60,
            heightMeters: 60,
          ...
          final updated = fakeTerritoryRepository.territories.firstWhere(
            (t) => t.userId == 'user-1',
          );
    ...
    test(
      ...
      () {
        fakeAsync((async) {
          ...
          final initialDefendedTime = DateTime.now().subtract(
          ...
          fakeTerritoryRepository.territories.add(
            TerritoryEntity(
              ...
              polygons: [loopUser],
              areaSqMeters: 3600.0,
              lastDefendedAt: initialDefendedTime,
              isOwnedByCurrentUser: true,
            ),
          );
    ...
    test(
      ...
      () {
        fakeAsync((async) {
          ...
          final loopA = createRectangleLoop(
            startLat: 40.7120,
            startLng: -74.0060,
            widthMeters: 100,
            heightMeters: 100,
          ...
          final loopB = createRectangleLoop(
            startLat: 40.7125,
            startLng: -74.0055,
            widthMeters: 100,
            heightMeters: 100,
          ...
          final userA = fakeTerritoryRepository.territories.firstWhere(
            (t) => t.userId == 'user-1',
          );
          ...
          final loopRetake = createRectangleLoop(
            startLat: 40.7120,
            startLng: -74.0060,
            widthMeters: 120,
            heightMeters: 120,
          ...
          final updatedUserA = fakeTerritoryRepository.territories.firstWhere(
            (t) => t.userId == 'user-1',
          );
    ...
    test(
      ...
      () {
        fakeAsync((async) {
          ...
          final decayStartTime = DateTime.now().subtract(
            const Duration(days: 6),
          );
          fakeTerritoryRepository.territories.add(
            TerritoryEntity(
...
class FakeViewerLocationNotifier extends ViewerLocationNotifier {
  @override
  GeoPointEntity? build() => GeoPointEntity(
    latitude: 40.7128,
    longitude: -74.0060,
    timestamp: DateTime.now(),
  );
